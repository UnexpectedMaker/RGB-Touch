...
void Share_ESPNOW::onDataRecv(const uint8_t *mac_addr, const uint8_t *data, int data_len)
{
	...
	if (data_len == 10 && strncmp((const char *)data, "RGBT_PING!", 10) == 0)
	{
		...
		if (!known_peer)
		{
			...
			memset(&new_peer.peerInfo, 0, sizeof(new_peer.peerInfo));
			memcpy(new_peer.peerInfo.peer_addr, mac_addr, 6);
			new_peer.peerInfo.channel = 0; // Use default channel
			new_peer.peerInfo.encrypt = false;
	...
	else if (data_len < 180) // we'll need to capture incoming messages that are not share packets
	{
	}
	else
	{
...
void Share_ESPNOW::find_peers(bool force)
{
	if (force || millis() - next_peer_broadcast > 10000)
	{
		...
		if (recorder.is_receiving || recorder.is_recording)
			return;
...
void Share_ESPNOW::onDataSentWrapper(const uint8_t *mac_addr, esp_now_send_status_t status)
{
	...
	// share_espnow.onDataSent(mac_addr, status);
}
...
void Share_ESPNOW::onDataRecvWrapper(const uint8_t *mac_addr, const uint8_t *data, int data_len)
{
	...
	// share_espnow.onDataRecv(mac_addr, data, data_len);
}
