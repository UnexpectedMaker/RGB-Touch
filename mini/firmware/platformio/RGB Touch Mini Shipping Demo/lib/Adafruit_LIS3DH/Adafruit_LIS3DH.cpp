 ...
 */
Adafruit_LIS3DH::Adafruit_LIS3DH(int8_t cspin, SPIClass *theSPI, uint32_t frequency)
{
	_cs = cspin;
	_mosi = -1;
	_miso = -1;
 ...
 *           frequency of the SPI interface
 */
Adafruit_LIS3DH::Adafruit_LIS3DH(int8_t cspin, int8_t mosipin, int8_t misopin, int8_t sckpin, uint32_t frequency)
{
	_cs = cspin;
	_mosi = mosipin;
	_miso = misopin;
...
void Adafruit_LIS3DH::setINTpolarity(uint8_t val)
{
	...
	Adafruit_BusIO_RegisterBits int_data = Adafruit_BusIO_RegisterBits(&_ctrl1, 1, 0);
	...
	Adafruit_BusIO_RegisterBits ths_data = Adafruit_BusIO_RegisterBits(&_ctrl2, 7, 0);
	ths_data.write(127);
