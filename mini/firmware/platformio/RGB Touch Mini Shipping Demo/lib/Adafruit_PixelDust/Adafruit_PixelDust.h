...
typedef uint32_t grain_col_t;	///< Grain Colour
...
typedef struct
{
		...
		grain_col_t color_start = 0;
		grain_col_t color_current = 0;
} Grain;
...
class Adafruit_PixelDust
{
	public:
		...
		*/
		Adafruit_PixelDust(dimension_t w, dimension_t h, grain_count_t n, uint8_t s, uint8_t e = 128, bool sort = false);
		...
		*/
		bool setPosition(grain_count_t i, dimension_t x, dimension_t y, uint32_t col);
		...
		*/
		void getPosition(grain_count_t i, dimension_t *x, dimension_t *y, grain_col_t *c) const;
		...
		*/
		void randomize(uint32_t col, bool rand_col);
		...
		*/
		...
		void attract(int16_t posx, int16_t posy, grain_col_t target);
		void reposition();
		grain_col_t Color(uint8_t r, uint8_t g, uint8_t b);
		grain_col_t ColorLerp(grain_col_t a, grain_col_t b);
